...
void SystemUI::printInitializationMessages()
{
    for (size_t i{}; i < systems_k.size(); ++i)
    {
        Utils::printLabel(LanguageManager::getText(systems_k[i]), COLORS::YELLOW);
        Utils::printMessageNewLine(LanguageManager::getText(inits_k[i]));
...
{
    Utils::printColorfulMessageNewLine(LanguageManager::getText("EMPTY"),
                                       COLORS::RED);
}
...
{
    const std::string_view color{temp <= warningTemp
                                     ? COLORS::WHITE
                                     : temp >= dangerTemp
                                     ? COLORS::RED
                                     : COLORS::L_YELLOW};
...
{
    const std::string_view color{obj->getFuel() <= Vehicle::getLowFuel()
                                     ? COLORS::RED
                                     : obj->getFuel() > Vehicle::getMediumFuel() &&
                                       obj->getFuel() <= Vehicle::getMediumFuel()
                                     ? COLORS::L_YELLOW
                                     : obj->getFuel() >= Vehicle::getHighFuel()
                                     ? COLORS::GREEN
                                     : COLORS::WHITE};
